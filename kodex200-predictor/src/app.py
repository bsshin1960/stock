...
class StockPredictorApp:
    ...
    def setup_ui(self):
        ...
        self.consensus_box = ft.Container(
            ...
            border=ft.Border.all(2, "#40C4FF"), width=631, height=99,
        ...
        self.top10_box = ft.Container(
            ...
            border=ft.Border.all(1, "#78909C"), width=309, height=99,
            on_hover=self.handle_body_hover,
        )
        ...
        self.macro_section_text = ft.Text("주가 변경 인자 (30.0%)", size=13, color="#C084FC", weight=ft.FontWeight.BOLD)
        ...
        body = ft.Column([
            ft.Row([
                ft.Column([self.title_row, self.subtitle_label], spacing=4, expand=True),
                theme_and_run_col
        ...
        if self.scroll_mode == "scrollbar":
            ...
            self.vertical_scroll = self.vertical_scroll_stack
        else:
    ...
    def _mk_ai_card(self, model_name, display_name, color):
        ...
        lr = ft.Text("대기 중...", size=11, color="#475569", no_wrap=True, style=ft.TextStyle(height=2.0))
        ...
        c = ft.Container(
            content=ft.Column([
                ...
                price_pct_row,
                ft.Container(
                    content=ft.Row([lr], scroll=ft.ScrollMode.AUTO),
                    expand=True,
    ...
    def open_app_settings_dialog(self, e):
        ...
        dlg = ft.AlertDialog(
            ...
            content=ft.Column([
                ft.Text("▣ 스크롤바 설정", size=14, weight=ft.FontWeight.BOLD, color=accent),
                scroll_radio_group,
    ...
    def update_scroll_mode(self, mode: str):
        ...
        if mode == "scrollbar":
            ...
            self.scroll_rail.visible = True
            self.vertical_scroll = self.vertical_scroll_stack
            self.scrollable_body.controls[0] = self.vertical_scroll_stack
        else:
    ...
    def update_weights_label(self):
        try:
            ...
            if hasattr(self, "macro_section_text"):
                self.macro_section_text.value = f"주가 변경 인자 ({w_macro*100:.1f}%)"
            ...
            if hasattr(self, "macro_section_text"):
                self.macro_section_text.value = "주가 변경 인자 (30.0%)"
            if hasattr(self, "news_title_text"):
        ...
        try:
            ...
            for item in history:
                ...
                if pred_time < datetime.time(9, 0, 0):
                    target_days = [d for d in trading_days if d >= pred_dt]
                else:
                    target_days = [d for d in trading_days if d > pred_dt]
                ...
                next_trading_day = min(target_days)
    ...
    def record_prediction_in_history(self, timestamp: str, current_price: int, pred_dir: str, target_price: int, pct: float, ai_results: dict, consensus_result: dict, macro_data: dict):
        ...
        new_entry = {
            ...
            "before_macro_weights": _get_configured_macro_weights(),
